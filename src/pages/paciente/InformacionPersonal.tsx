...
function InformaciónPersonal() {
  ...
  async function handleUpdate(e: IPersonaUpdate) {
    ...
    if (rsta == undefined) return; 
      //como paciente
      actualizarPaciente(rsta)
     
  }

  function actualizarPaciente(e : IPersonaResponse){
    setPacienteInfo((prev) => ({
      ...prev, ...e,
      telefonoEmergencia : prev?.telefonoEmergencia || '',
     nombreEmergencia : prev?.nombreEmergencia || ''
     }));
  }
